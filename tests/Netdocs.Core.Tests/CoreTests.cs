...
public class MacrosTests
{
    ...
    public async Task FileUri_ResolvesPageUrl()
    {
        ...
        var result = await new Netdocs.Plugins.MacrosPlugin()
    ...
    [Fact]
    public async Task Ebay_RendersAffiliateLink()
    {
        var page = new Page { SourcePath = "b", RelativePath = "index.md", Url = "" };
        var result = await new Netdocs.Plugins.MacrosPlugin()
            .ProcessAsync(page, "{{ ebay(\"Buy it\", \"https://ebay.com/itm/1\") }}", Site(page), default);

        Assert.Contains("class=\"ebay-affiliate-link\"", result);
        Assert.Contains("href=\"https://ebay.com/itm/1\"", result);
        Assert.Contains(">Buy it</a>", result);
    }
    ...
    public async Task IgnoreMacros_LeavesMarkdownUntouched()
    {
        ...
        const string md = "{{ ebay(\"x\", \"y\") }}";
        var result = await new Netdocs.Plugins.MacrosPlugin().ProcessAsync(page, md, Site(page), default);
